...
if (
  parseBool(process.env.LOGGING_ENABLE) ||
  config.appConfig.apiLoggingEnable
) {
...
app.use("/admin/v1/", [apiLimiter, routes]);

app.get("/", (req, res) => {
  res.json({
    status: 200,
    message: "Service is OK.",
    data: "pdate with Build version 03.16.00",
  });
  res.end();
});
...
app.all("*", function (req, res) {
  console.log(req);
  const err = new Error(
    `RouteError: Cannot ${req.method} ${req.originalUrl}, No HTTP resource was found that matches the request.`
  );
  err.statusCode = 404;
...
app.listen(process.env.PORT || config.appConfig.port, () => {
  console.log(
    "Server is listening on port " + (process.env.PORT || config.appConfig.port)
  );
});
